...
class PlayerStats(BaseModel):
    hp: float = Field(
        ...,
        description="Máu của người chơi",
        gt=0,
        json_schema_extra={"example": 120.0},
    )
    atk: float = Field(
        ..., description="Sát thương gây ra", json_schema_extra={"example": 90.0}
    )
    defence: float = Field(
        ..., description="Chỉ số phòng thủ", json_schema_extra={"example": 30.0}
    )
    critPercentage: float = Field(
        ...,
        description="Tỉ lệ chí mạng",
        ge=0.0,
        le=100.0,
        json_schema_extra={"example": 5.05},
    )
    critDamage: float = Field(
        ..., description="Sát thương chí mạng", json_schema_extra={"example": 240.0}
    )
    dodge: float = Field(
        ..., description="Tỉ lệ né đòn", json_schema_extra={"example": 10.0}
    )
