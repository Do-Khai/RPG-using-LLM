...
class ChatRequest(BaseModel):
    ...
    message: str = Field(..., description="Lệnh người chơi vừa gửi", json_schema_extra={"example": "/choose 1"})
    all_region_data: List[Dict[str, Any]] = Field(..., description="Danh sách tất cả các vùng đất trong game và thông tin chi tiết.", json_schema_extra={"example": [{
		'code': 'VALORIA',
		'name': 'Thành Valoria',
    ...
    current_stats: Dict[str, Any] = Field(..., description="Các chỉ số hiện tại của người chơi.", json_schema_extra={"example": {
        "hp": 120.0, "atk": 90.0, "def": 30.0, "crit": 5.05, "critDamage": 240.0, "dodge": 10.0, "speed": 24.0
    }})
    next_stats: Dict[str, Any] = Field(..., description="Các chỉ số của người chơi khi lên cấp độ tiếp theo.", json_schema_extra={"example": {
        "hp": 135.0, "atk": 98.0, "def": 35.0, "crit": 5.50, "critDamage": 245.0, "dodge": 11.0, "speed": 25.0
    }})
    ...
    last_story: Optional[Dict[str, Any]] = Field(None, description="Đối tượng JSON của câu chuyện/nhiệm vụ cuối cùng mà AI đã tạo ra.", json_schema_extra={"example": {
        "type": "STORY",
