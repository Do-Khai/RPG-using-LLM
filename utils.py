...
class KeyManager:
    def __init__(self, keys: list, cooldown_seconds: int = 3600):
        ...
        self._key_status = {
            key: {"available": True, "cooldown_until": 0} for key in keys
        }
    ...
    def set_key_cooldown(self, key: str):
        with self._lock:
            if key in self._key_status:
                logging.warning(
                    f"Key ...{key[-4:]} đang được đưa vào trạng thái cooldown trong {self._cooldown_seconds} giây."
                )
                self._key_status[key]["available"] = False
                self._key_status[key]["cooldown_until"] = (
...
async def call_ollama(payload: dict) -> dict:
    ...
    raise Exception(
        "Tất cả các API key đều đã thất bại hoặc đang trong thời gian cooldown."
    )
